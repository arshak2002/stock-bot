...
class IntradayBot:
    ...
    def __init__(self, symbols: list):
        ...
        self._state_file = os.path.join(base_dir, "daily_state.json")
        ...
        # Load persisted daily state (survives restarts)
        self.signaled_today, self.trade_count = self._load_daily_state()
        
        # Schedule morning brief
        schedule.every().day.at("09:14").do(self.send_morning_brief)
        schedule.every().day.at("09:15").do(self._reset_daily_state)  # fresh slate each morning
        # V5.2: Refresh sector strength every 30 minutes
        schedule.every(30).minutes.do(self._refresh_sector_strength)

    def _load_daily_state(self):
        """Load today's traded symbols and trade count from disk. Resets if it's a new day."""
        today = datetime.now().strftime("%Y-%m-%d")
        try:
            with open(self._state_file, "r") as f:
                state = json.load(f)
            if state.get("date") == today:
                signaled = set(state.get("signaled_today", []))
                count = state.get("trade_count", {"BUY": 0, "SELL": 0})
                print(f"[*] Restored daily state: {len(signaled)} symbols traded, {sum(count.values())} total trades")
                return signaled, count
        except Exception:
            pass
        # New day or no state file
        return set(), {"BUY": 0, "SELL": 0}

    def _save_daily_state(self):
        """Persist daily state to disk so restarts don't reset the trade guard."""
        today = datetime.now().strftime("%Y-%m-%d")
        state = {
            "date": today,
            "signaled_today": list(self.signaled_today),
    ...
    def _reset_daily_state(self):
        """Called at 9:15 AM each morning to clear the previous day's state."""
        self.signaled_today = set()
        self.trade_count = {"BUY": 0, "SELL": 0}
        self._save_daily_state()
        print(f"[*] Daily state reset for new session")
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                self.cfg = load_config()
                cfg = self.cfg
                self.circuit.update_cfg(cfg)

                # Hard cap: stop scanning if max trades for the day reached
                max_trades = cfg["session"]["max_trades_per_day"]
                total_trades = sum(self.trade_count.values())
                if total_trades >= max_trades:
                    print(f"[{datetime.now().strftime('%H:%M:%S')}] ✅ Max trades/day reached ({total_trades}/{max_trades}). Resting.")
                    time.sleep(300)  # check again in 5 min (in case config is hot-reloaded)
                    continue
                ...
                for sym in self.symbols:
                    ...
                    try:
                        ...
                        data["global_bias"] = fetch_global_market_bias()
                        ...
                        data.setdefault("global_bias", {"bias": "NEUTRAL", "score_mod": 0.0})
                    ...
                    if result["signal"] != "NO TRADE":
                        # Slippage check — use actual historical win rate for true edge
                        slippage_pct = SlippageModel.get_slippage_pct(data["symbol"], cfg)
                        exp_now = self.expectancy.compute()
                        hist_win_rate = exp_now.get("win_rate", 50) / 100
                        true_edge = SlippageModel.compute_true_edge(
                            result["entry"], result["target"], result["stop_loss"],
                            slippage_pct, result["signal"], win_rate=hist_win_rate,
                        )
                        ...
                        # ── LOCK OUT this symbol NOW before any further processing ──
                        # This must happen before logging/alerting so a crash cannot cause re-entry
                        self.signaled_today.add(data["symbol"])
                        self.trade_count[result["signal"]] += 1
                        self._save_daily_state()  # persist to disk — survives restarts
                        ...
                        if data["symbol"] not in self.active_tranches:
                            ...
                            print(f"[TRANCHE] {data['symbol']} ({dir_str}) | SL=₹{result['stop_loss']} TGT=₹{result['target']}")
